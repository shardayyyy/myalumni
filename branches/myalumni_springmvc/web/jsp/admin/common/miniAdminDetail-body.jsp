...
<c:choose>
	<c:when test="${ isOnline == 'Y'}">
		<font class="blueText">Welcome <c:out value="${USER_CONTAINER.token.firstName}"/> <c:out value="${USER_CONTAINER.token.lastName}"/>&nbsp;&nbsp;[<a href="<c:out value="${LOGOUT}"/>">Logout</a>]&nbsp;<br>[<a href="<c:out value="${mainsite}"/>">Member Index</a>]</font>
	 </c:when>
	 <c:otherwise>
