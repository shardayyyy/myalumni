...
import net.naijatek.myalumni.persistence.hibernate.iface.MessengerDao;
import net.naijatek.myalumni.service.IMessengerService;
...
/*    public MessengerServiceImpl(MessengerDao messengerDao) {
	...
	public void saveAll(List<MessengerVO> list, String memberId) throws MyAlumniException{
		messengerDao.saveAll(list, memberId);
	}

    @Transactional
	public void hardDeleteAll(String memberId) throws MyAlumniException{
		messengerDao.hardDeleteAll(memberId);
	}

    @Transactional(readOnly = true)
	public List<XlatDetailVO> getActiveMemberMessengers(String memberId){
		List<XlatDetailVO> details = new ArrayList<XlatDetailVO>();
		List<MessengerVO> messengers = messengerDao.getActiveMemberMessengers(memberId);
		for(MessengerVO messenger: messengers){
			details.add(messenger.getMessenger());
		}
		return details;
	}
