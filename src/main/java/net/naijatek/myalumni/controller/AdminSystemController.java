...
import net.naijatek.myalumni.entity.*;
...
import net.naijatek.myalumni.controller.exceptions.MyAlumniBaseException;
import net.naijatek.myalumni.modules.admin.presentation.form.SystemConfigVO;
import net.naijatek.myalumni.controller.helper.BaseSystemHelper;
import net.naijatek.myalumni.controller.helper.DropDownCacheBuilder;
import net.naijatek.myalumni.entity.SystemVO;
...
public class AdminSystemController extends MyAlumniBaseController {
...
/*	public MaintainSystemModuleAction(IErrorLogService logService,
    ...
    public ModelAndView prepareUpdateOrgAboutUs(@ModelAttribute("member") SystemConfigVO orgInfoVO, BindingResult result, SessionStatus status) throws
        ...
        ModelAndView mv = new ModelAndView(BaseConstants.FWD_SUCCESS);
        //saveToken(request);
        SystemConfigVO orgInfoForm = (SystemConfigVO) form;
    ...
    public ModelAndView prepareUpdateRssFeed(@ModelAttribute("member") MemberVO memberVO, BindingResult result, SessionStatus status) throws
        ...
        saveToken(request);
        SystemConfigVO rssForm = (SystemConfigVO) form;
    ...
    public ModelAndView prepareUpdateTwitterCred(@ModelAttribute("member") MemberVO memberVO, BindingResult result, SessionStatus status) throws
        ...
        saveToken(request);
        SystemConfigVO configForm = (SystemConfigVO) form;
        SystemConfigVO systemSetup = systemConfigService.getSystemConfig();
    ...
    public ModelAndView prepareUpdateDormitory(@ModelAttribute("member") MemberVO memberVO, BindingResult result, SessionStatus status) throws
        ...
        saveToken(request);
        SystemConfigVO sysForm = (SystemConfigVO) form;
    ...
    public ModelAndView prepareUpdateSessionTimeOut(@ModelAttribute("member") MemberVO memberVO, BindingResult result, SessionStatus status) throws
        ...
        saveToken(request);
        SystemConfigVO systemForm = (SystemConfigVO) form;
    ...
    public ModelAndView deleteDatabaseBackup(@ModelAttribute("member") MemberVO memberVO, BindingResult result, SessionStatus status) throws
        ...
        ActionMessages msgs = new ActionMessages();
        SystemVO sysVO = (SystemVO) form;
        String file = getSysProp().getValue("BACKUP.FILEPATH") + sysVO.getLogFileName();
        File sqlFile = new File(file);

        if (sqlFile.exists() && sqlFile.isFile()) {
            sqlFile.delete();
            msgs.add(BaseConstants.INFO_KEY, new ActionMessage("message.successfullydeleted", sysVO.getLogFileName()));
            saveMessages(request, msgs);
        } else {
    ...
    public ModelAndView viewDatabaseBackup(@ModelAttribute("member") MemberVO memberVO, BindingResult result, SessionStatus status) throws
        ...
        if (sysVO.getLogType() == null || sysVO.getLogType().length() == 0) {
            sysVO.setLogType("ALL");
        }

        File file = new File(getSysProp().getValue("BACKUP.FILEPATH") + sysVO.getLogFileName());
        sysVO.setLineCount("5000");
        String[] contentLog = FileUtil.getLastLines(file, Integer.parseInt(sysVO.getLineCount()), sysVO.getLogType());
...
//		
    ...
    public ModelAndView deleteSystemLog(@ModelAttribute("member") MemberVO memberVO, BindingResult result, SessionStatus status) throws
        ...
        ActionMessages msgs = new ActionMessages();
        SystemVO sysVO = (SystemVO) form;
        String file = getSysProp().getValue("LOGFILE.FILEPATH") + sysVO.getLogFileName();
    ...
    public ModelAndView viewSystemLogs(@ModelAttribute("member") MemberVO memberVO, BindingResult result, SessionStatus status) throws
        ...
        int lineCount = 0;
        try {
            lineCount = Integer.parseInt(sysVO.getLineCount()); // 25, 50 100 200 400 800)
        } catch (Exception e) {
            if (lineCount == 0) {
                lineCount = 50;
            }
        ...
        File logFile = new File(getSysProp().getValue("LOGFILE.FILEPATH") + sysVO.getLogFileName());
        sysVO.setLineCount(String.valueOf(lineCount));
        String[] contentLog = FileUtil.getLastLines(logFile, lineCount, sysVO.getLogType());

        String humanSize = FileUtil.getHumanSize(logFile.length());
        sysVO.setLogFileSize(String.valueOf(logFile.length()));
        sysVO.setLogFileHumanSize(humanSize);
...
/*	public ModelAndView viewLog2(ActionMapping mapping, ActionForm form,
        ...
        SystemVO sysForm = (SystemVO)form;
...
/*
    ...
    public ModelAndView maintainScroll(@ModelAttribute("member") MemberVO memberVO, BindingResult result, SessionStatus status) throws
        ...
        private void getReminisceHelper (HttpServletRequest request){
            ...
            ServletContext sCtx = request.getSession().getServletContext();
            ActionMessages msgs = new ActionMessages();
            SystemConfigVO systemConfigForm = (SystemConfigVO) form;
            SystemConfigVO systemConfigVO = new SystemConfigVO();
